...
The backend sends your question and the table schema to an LLM (GPT-4o-mini by default), gets back a SQL query, runs it against your data, and returns the results — with a plain-English explanation and a chart type suggestion.
...
```json
{
  ...
  "explanation": "This query groups all orders by product category and sums up the revenue for each. Electronics came out on top, driven by higher unit prices and multiple purchases.",
  "chart_type": "bar",
  "columns": ["category", "total_revenue"],
  "rows": [["Electronics", 999.92], ["Sports", 529.89], ...],
  "row_count": 5,
  "exec_time_ms": 3
}
```
...
3. Sends your question to the LLM with `temperature=0.1` (low randomness — we want consistent SQL, not creative SQL)
...
The tests don't require a real OpenAI key — they test the API, upload flow, and input validation independently of the LLM.
